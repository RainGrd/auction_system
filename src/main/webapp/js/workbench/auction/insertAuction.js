$(function () {
     ...
     * 保存按钮点击事件
     */
    $("#saveAuctionBtn").on('click', function () {
        let imageUrl = $('#image_url').val();
        let auctionPic = imageUrl.substr(0, imageUrl.lastIndexOf('.'));
        console.log(auctionPic)
        /*收集参数*/
        let $auction = {
            auctionName: $('#name').val(),
            auctionStartPrice: $('#startPrice').val(),
            auctionUpset: $('#endPrice').val(),
            auctionStartTime: $('#startDate').val(),
            auctionEndTime: $('#endDate').val(),
            auctionPic: auctionPic,
            auctionPicType: imageUrl.substr(imageUrl.lastIndexOf('.')),
            auctionDesc: $('#description').val(),
        }
        /*表单验证*/
        //非空验证
        ...
        //数字验证
        if (!checkNumber($auction.auctionStartPrice)) {
            $('#startPrice').siblings("div").removeClass('hide');
            return false;
        }
        if (!checkNumber($auction.auctionUpset)) {
            $('#endPrice').siblings("div").removeClass('hide');
            return false;
        }
        //时间验证
        if (!checkDataTime($auction.auctionStartTime)) {
            $('#startDate').siblings("div").removeClass('hide');
            return false;
        }
        if (!checkDataTime($auction.auctionEndTime)) {
            $('#endDate').siblings("div").removeClass('hide');
            return false;
        }
        /*发送参数*/
        $.ajax({
            url: "workbench/auction/saveAuction.do",
            type: 'post',
            dataType: 'json',
